...
useEffect(() => {
    ...
    const handleDelete = async() => {
        const res = await fetch("http://localhost:3001/api/users/" + userToDelete._id, { method: "DELETE" });
        if (res.ok){
    ...
    const handleEdit = async () => {
        ...
        const res = await fetch("http://localhost:3001/api/users/" + userToEdit._id, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
        });
        if (res.ok) {
            setUsers(users.map(u => u._id === userToEdit._id ? { ...u, ...body } : u));
            setUserToEdit(null);
        }
    };

    const handlePasswordReset = async () => {
        await fetch("http://localhost:3001/api/users/" + userToEdit._id + "/reset-password", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
                                ...
                                <tr key={user._id}>
                                    ...
                                    <td>
                                        <button className="edit-button" onClick={() => {
                                            setUserToEdit(user);
                                            setEditForm({ userName: user.userName, isAdmin: user.isAdmin, isBanned: user.isBanned || false });
                                        }}>
                                            <MdEdit/>
                                        </button>
            ...
            {userToDelete && (
                <div className="delete-confirmation">
                    <div className="confirmation-box">
                        <p>Are you sure you want to delete {userToDelete.userName}'s account?</p>
                ...
                {userToEdit && (
                    <div className="edit-overlay">
                        <div className="edit-box">
                            ...
                            <label>Banned:</label>
                            <input type="checkbox" checked={editForm.isBanned} onChange={(e) => setEditForm({...editForm, isBanned: e.target.checked})}/>
