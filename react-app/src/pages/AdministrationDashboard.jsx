...
export const AdministrationDashboard = () => {
  ...
  const handleEdit = async () => {
    ...
    if (res.ok) {
      ...
      if (
        body.isBanned !== (userToEdit.isBanned || false) &&
        searchTerm.trim()
      ) {
  ...
  const handlePasswordReset = async () => {
    await fetch("/api/users/" + userToEdit._id + "/reset-password", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
            ...
            <>
              <h2>
                Users (
                {
                  users
                    .filter(
                      (u) =>
                        (userFilters.active && !u.isBanned) ||
                        (userFilters.banned && u.isBanned),
                    )
                    .filter(
                      (u) =>
                        (userFilters.admin && u.isAdmin) ||
                        (userFilters.user && !u.isAdmin),
                    ).length
                }
                )
              </h2>
              ...
              >
                ...
                <label>
                  <input
                    type="checkbox"
                    checked={userFilters.active}
                    onChange={() =>
                      setUserFilters((f) => ({ ...f, active: !f.active }))
                    }
                  />{" "}
                  Active
                </label>
                <label>
                  <input
                    type="checkbox"
                    checked={userFilters.banned}
                    onChange={() =>
                      setUserFilters((f) => ({ ...f, banned: !f.banned }))
                    }
                  />{" "}
                  Banned
                </label>
                ...
                <label>
                  <input
                    type="checkbox"
                    checked={userFilters.admin}
                    onChange={() =>
                      setUserFilters((f) => ({ ...f, admin: !f.admin }))
                    }
                  />{" "}
                  Admin
                </label>
                <label>
                  <input
                    type="checkbox"
                    checked={userFilters.user}
                    onChange={() =>
                      setUserFilters((f) => ({ ...f, user: !f.user }))
                    }
                  />{" "}
                  User
                </label>
              ...
              >
                ...
                <label>
                  <input
                    type="checkbox"
                    checked={eventFilters.byName}
                    onChange={() =>
                      setEventFilters((f) => ({ ...f, byName: !f.byName }))
                    }
                  />{" "}
                  Event Name
                </label>
                <label>
                  <input
                    type="checkbox"
                    checked={eventFilters.byOwner}
                    onChange={() =>
                      setEventFilters((f) => ({ ...f, byOwner: !f.byOwner }))
                    }
                  />{" "}
                  Owner
                </label>
                <label>
                  <input
                    type="checkbox"
                    checked={eventFilters.byDate}
                    onChange={() =>
                      setEventFilters((f) => ({ ...f, byDate: !f.byDate }))
                    }
                  />{" "}
                  Date
                </label>
                ...
                <tbody>
                  {events.map((event) => (
                    <tr key={event._id}>
                      ...
                      {eventFilters.byOwner && (
                        <td>{event.ownerId?.userName || event.ownerId}</td>
                      )}
                      {eventFilters.byDate && (
                        <td>
          ...
          <div className="edit-box">
            ...
            >
              ...
              <button
                ...
                style={{
                  backgroundColor: editForm.isAdmin ? "#7c3aed" : "#e5e7eb",
                  color: editForm.isAdmin ? "white" : "#111",
                  border: "none",
                  borderRadius: "6px",
              ...
              <button
                onClick={() => {
                  ...
                  if (window.confirm(msg))
                    setEditForm({ ...editForm, isBanned: !editForm.isBanned });
                }}
                style={{
                  backgroundColor: editForm.isBanned ? "#dc2626" : "#e5e7eb",
                  color: editForm.isBanned ? "white" : "#111",
                  border: "none",
                  borderRadius: "6px",
                  padding: "0.3rem 0.8rem",
                  cursor: "pointer",
            ...
            {(editForm.userName !== userToEdit.userName ||
              editForm.isAdmin !== userToEdit.isAdmin ||
              editForm.isBanned !== (userToEdit.isBanned || false)) && (
              <p
                style={{
                  color: "#dc2626",
                  fontWeight: "bold",
